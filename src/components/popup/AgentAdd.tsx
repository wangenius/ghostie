...
export function AgentAdd() {
  ...
  const handleSubmit = async () => {
    try {
      await invoke("add_agent", { agent: agent });
  ...
  const addEnvVar = () => {
    setAgent({
      ...agent,
      env: [...agent.env, { name: "", value: "", description: "" }]
    });
  };
  ...
  return (
    <div className="app-container flex flex-col h-screen bg-background">
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {activeTab === "env" && (
          <div className="space-y-4">
            {agent.env.map((env, index) => (
              <div key={index} className="p-4 bg-secondary rounded-md space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium">环境变量 {index + 1}</h3>
                  <button
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newEnv = [...agent.env];
                      newEnv[index].description = e.target.value;
                      setAgent({ ...agent, env: newEnv });
                    }}
                    className="w-full h-9 px-3 bg-secondary/50 rounded-md text-sm focus:bg-secondary/80 transition-colors outline-none"
                    placeholder="描述"
