...
function registerWindowHandlers(): void {
  ...
  ipcMain.handle(
    "open-window",
    async (_, params: { name: string; query: any; config: any }) => {
      // TODO: 实现打开新窗口的功能
...
function registerNodeHandlers(): void {
  ...
  ipcMain.handle(
    "plugin-execute",
    async (_, command: string, _event: string[]) => {
      try {
...
function registerPluginFSHandlers(): void {
  ...
  ipcMain.handle(
    "plugin-save-content",
    async (_, params: { id: string; content: string }) => {
      try {
  ...
  ipcMain.handle("plugin-list", async () => {
    try {
      ...
      return files.map((file) => ({
        name: file,
        path: file,
        type: fs.statSync(join(pluginDir, file)).isDirectory()
          ? "directory"
          : "file",
      }));
    } catch (error) {
